...
//Here scene is that we don't need to hear for click events rather we would be listening for the form submit events, so we dont require btn but require form.;
...
form.addEventListener("submit", (e) => {
  ...
  if (val >= 0 && val < 10) {
    ...
    let v = Math.floor(Math.random() * text.length)
    result.innerHTML = `<p class="result">${text[v]}</p>`;
  }
})
